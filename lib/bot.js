...
import emojis from '../assets/emoji.json' assert { type: 'json' };
import { validateChannelMapping } from './validators.js';
import { highlightUsername } from './helpers.js';
...
class Bot {
  ...
  sendToSlack(author, channel, text) {
    ...
    if (slackChannelName) {
      ...
      this.slack.web.conversations.members({channel: slackChannel.id}).then((resp) => {
